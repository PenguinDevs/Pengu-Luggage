...
function DataStore:Save()
	...
	if self.value ~= nil then
		...
		if not success then
			...
			--spawn(function() require(script.SentryLogger):log(tostring(problem)) end)
		...
		if RunService:IsStudio() then print("saved "..self.Name) end
	end
end
...
function DataStore2:__call(dataStoreName, player, ignoreCheck)
	...
	elseif combinedDataStoreInfo[dataStoreName] then
		local dataStore = DataStore2(combinedDataStoreInfo[dataStoreName], player, ignoreCheck)

		dataStore:BeforeSave(function(combinedData)
			for key in pairs(combinedData) do
				if combinedDataStoreInfo[key] then
					local combinedStore = DataStore2(key, player, ignoreCheck)
