...
local UpdateFreecam do
	...
	function UpdateFreecam()
		...
		-- Edited by PenguinDevs from here, just creating a magnitude boundary
		if c.p.magnitude < 80 and c.p.Y > 0 then
			camera.CFrame = c
		else
			camera.CFrame = CFrame.new(camCFrame.p) * CFrame.Angles(0, stateRot.y, 0) * CFrame.Angles(stateRot.x, 0, 0)
		end
...
--local freeBool = script.Parent.MainScript.freeCam
--freeBool.Changed:connect(function()
--	if freeBool.Value == true then
--		EnterFreecam()
--	else
--		ExitFreecam()
--	end
...
function freecam:EnterFreecam()
	EnterFreecam()
end

function freecam:ExitFreecam()
	ExitFreecam()
end
		...
		if input.KeyCode == Enum.KeyCode.C then
			if script.Parent.Main.Visible == true then
				if freeCamEnabled then
					...
					script.Parent.MainScript.freeCam.Value = true
					script.Parent.Main.Lock.Image = "rbxassetid://398764086"
				end
			end
		end
	end
