...
if (...) then
            ...
            for y = self._grid._max_y, self._grid._min_y, -1 do
                ...
                if self._grid:isWalkableAt(x, y, self._walkable) then
                    ...
                    if nr and nrd and nd then
                        ...
                        m = (nd._clearance[self._walkable] or 0) < m and
                                (nd._clearance[self._walkable] or 0) or m
                        m = (nr._clearance[self._walkable] or 0) < m and
                                (nr._clearance[self._walkable] or 0) or m
    ...
    function Pathfinder:setGrid(grid)
        assert(Assert.inherits(grid, Grid),
               'Wrong argument #1. Expected a \'grid\' object')
        self._grid = grid
    ...
    function Pathfinder:getGrid() return self._grid end
    ...
    function Pathfinder:setWalkable(walkable)
        assert(Assert.matchType(walkable, 'stringintfunctionnil'),
               ('Wrong argument #1. Expected \'string\', \'number\' or \'function\', got %s.'):format(
                   type(walkable)))
    ...
    function Pathfinder:getWalkable() return self._walkable end
    ...
    function Pathfinder:getFinder() return self._finder end
    ...
    --   print(i, finderName)
    -- end
    function Pathfinder:getFinders() return Utils.getKeys(Finders) end
    ...
    function Pathfinder:setHeuristic(heuristic)
        assert(Heuristic[heuristic] or (type(heuristic) == 'function'),
    ...
    function Pathfinder:getHeuristic() return self._heuristic end
    ...
    --   ...
    -- end
    function Pathfinder:getHeuristics() return Utils.getKeys(Heuristic) end
    ...
    --   ...
    -- end
    function Pathfinder:getModes() return Utils.getKeys(searchModes) end
    ...
    function Pathfinder:setTunnelling(bool)
        assert(Assert.isBool(bool),
               ('Wrong argument #1. Expected boolean, got %s'):format(type(bool)))
        self._tunnel = bool
        return self
    end
    ...
    function Pathfinder:getTunnelling() return self._tunnel end
    ...
    function Pathfinder:getPath(startX, startY, endX, endY, clearance)
        ...
        local _endNode = Finders[self._finder](self, startNode, endNode,
                                               clearance, toClear)
        if _endNode then
