...
if (...) then
    ...
    local lineOfSight = function(node, neighbour, finder, clearance)
        ...
        while true do
            if not finder._grid:isWalkableAt(x0, y0, finder._walkable,
                                             finder._tunnel, clearance) then
                return false
            end
    ...
    return function(finder, startNode, endNode, clearance, toClear,
                    overrideHeuristic)
        return astar_search(finder, startNode, endNode, clearance, toClear,
                            overrideHeuristic, computeCost)
    end
