...
if (...) then
    ...
    -- @treturn path a `path`
    -- @usage local p = Path()
    function Path:new() return setmetatable({_nodes = {}}, Path) end
    ...
    function Path:fill()
        ...
        while true do
            ...
            if (abs(dx) > 1 or abs(dy) > 1) then
                ...
                t_insert(self._nodes, i,
                         self._grid:getNodeAt(self._nodes[i - 1]._x + incrX,
                                              self._nodes[i - 1]._y + incrY))
                N = N + 1
            else
                i = i + 1
            end
    ...
    return setmetatable(Path,
                        {__call = function(self, ...) return Path:new(...) end})
end
